...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    return True if is_planet(m) else balanced(end(left(m))) and balanced(end(right(m))) and length(
        left(m)) * total_weight(end(left(m))) == length(right(m)) * total_weight(end(right(m)))
...
def has_path(t, phrase):
    ...
    if is_leaf(t):
        return False
    return bool(max(has_path(branch, phrase[1:]) for branch in branches(t)))
...
def div_interval(x, y):
    ...
    assert lower_bound(reciprocal_y) < upper_bound(reciprocal_y)
